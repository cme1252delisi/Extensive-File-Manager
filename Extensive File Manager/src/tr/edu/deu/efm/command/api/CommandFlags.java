...
/**
 * Encapsulates the parsing, storage, and retrieval of command-line flags
 * (options).
 * <p>
 * In command-line interfaces, flags (e.g., {@code -r}, {@code -f}) modify the
 * default behavior of a command. This class provides a centralized mechanism to
 * handle both isolated flags (e.g., {@code -r -f}) and POSIX-style compound
 * flags (e.g., {@code -rf}).
 * </p>
 * <p>
 * <b>Architectural Note:</b> Internally backed by a {@link java.util.HashSet},
 * this class automatically guarantees flag uniqueness. Redundant user inputs
 * like {@code -r -r} or {@code -rr} are gracefully collapsed into a single
 * active flag state.
 * </p>
 */
public class CommandFlags {
	 ...
	 * <p>
	 * If the provided string begins with a hyphen ({@code -}), the hyphen is
	 * ignored, and every subsequent character is registered as an independent
	 * active flag. Example: The input {@code "-rf"} will register both {@code 'r'}
	 * and {@code 'f'}.
	 * </p>
	 *
	 ...
	 * <p>
	 * This is the primary method utilized by command implementations (e.g.,
	 * {@code RmCommand}) to determine if they should execute alternative logic
